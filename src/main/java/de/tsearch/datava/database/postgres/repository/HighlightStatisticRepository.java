...
public interface HighlightStatisticRepository extends CrudRepository<Highlight, Long> {
    ...
    @Query("SELECT new de.tsearch.datava.database.postgres.data.WeekStatistics(extract (isodow from created_at) as weekday, COUNT(h)) FROM Highlight h WHERE h.broadcaster = :broadcaster GROUP BY weekday ORDER BY weekday")
